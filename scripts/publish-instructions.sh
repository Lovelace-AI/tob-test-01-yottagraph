...
# Commands that only make sense inside aether-dev (not distributed to tenants)
EXCLUDE_COMMANDS=(publish_instructions.md)
...
for f in "$ROOT_DIR"/commands/*.md; do
  ...
  base="$(basename "$f")"
  skip=false
  for exc in "${EXCLUDE_COMMANDS[@]}"; do
    [[ "$base" == "$exc" ]] && skip=true
  done
  $skip && { echo "    $base (skipped — dev-only)"; continue; }
  if $DRY_RUN; then
    echo "    $base"
  else
